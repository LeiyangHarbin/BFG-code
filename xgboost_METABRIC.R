...
sample1=rownames(pData(TNBC_ov1[[11]]))
na1=intersect(sample1,rownames(risk_score_train_glm))
risk_score=cbind(risk=risk_score_train_glm[na1,1],samples=na1)
risk_ST=merge(risk_score,Sta_Time_train,by="samples")#合并训练集的生存数据和risk数据
rownames(risk_ST)=risk_ST[,1]
risk_ST=risk_ST[,-1]
risk_ST[,1]=as.numeric(as.character(risk_ST[,1]))
...
high=rownames(risk_ST[which(risk_ST[,1]>= cut),])
low=rownames(risk_ST[which(risk_ST[,1]< cut),])
...
p5
ggsave("D:/SDQ/TNBC/NEW_data/NEW_data/建立预测模型/xgboost_METABRIC-import.pdf",width = 7,height = 7)
#xgb.plot.importance(importance_matrix =importance_train,top_n = 5)
pdf(file = "D:/SDQ/TNBC/NEW_data/NEW_data/建立预测模型/xgboost_METABRIC-shap.pdf",width = 11,height = 6)
...
pdf(file = "D:/SDQ/TNBC/NEW_data/NEW_data/建立预测模型/xgboost_METABRICroc.pdf",width = 7,height = 7)
par(family="serif",ps="15")
plot(xgboost_roc, print.auc=TRUE, auc.polygon=TRUE, 
     ...
     print.thres=TRUE,main='METABRIC cohort')
dev.off()
